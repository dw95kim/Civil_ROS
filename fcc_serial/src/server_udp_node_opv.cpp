...
int main(int argc, char** argv)
{
    ...
    init(argc, argv, "server_udp_node_opv");

    // assign node handler
    ros::NodeHandle nh;

    // for debugging
    printf("Initiate: server_udp_node_opv\n");

    // Publish Topic
    ros::Publisher FCC_Data_pub  = nh.advertise<fcc_serial::MsgStrRXttyO>("/OPV/FCC", 1);
    ros::Publisher GPS_pub         = nh.advertise<std_msgs::Float32MultiArray> ("/OPV/GPS"  ,1);
