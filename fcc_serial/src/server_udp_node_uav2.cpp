...
int main(int argc, char** argv)
{
    ...
    init(argc, argv, "server_udp_node_uav2");

    // assign node handler
    ros::NodeHandle nh;

    // for debugging
    printf("Initiate: server_udp_node_uav2\n");

    // Publish Topic
    ros::Publisher GPS_pub = nh.advertise<std_msgs::Float32MultiArray> ("/UAV2/GPS"  ,1);

    // Get port number
    // int UDP_PORTNUM;
    // nh.getParam("udp_portnum_uav1", UDP_PORTNUM);
    int UDP_PORTNUM = 14560;
    ...
    // for debugging
    printf("Terminate: comm_udp_node_uav2\n");
    close(StrUDP.Socket);
