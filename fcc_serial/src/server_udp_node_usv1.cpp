...
int main(int argc, char** argv)
{
    ...
    init(argc, argv, "server_udp_node_usv1");

    // assign node handler
    ros::NodeHandle nh;

    // for debugging
    printf("Initiate: server_udp_node_usv1\n");

    // Publish Topic
    ros::Publisher GPS_pub = nh.advertise<std_msgs::Float32MultiArray> ("/USV1/GPS"  ,1);

    // Get port number
    // int UDP_PORTNUM;
    // nh.getParam("udp_portnum_uav1", UDP_PORTNUM);
    int UDP_PORTNUM = 14570;
    ...
    while( ok() )
    {
        ...
        gps_data.data[4] = RX_buff[4]; // hor_vel
        gps_data.data[5] = RX_buff[5]; // ver_vel
    ...
    // for debugging
    printf("Terminate: comm_udp_node_usv1\n");
    close(StrUDP.Socket);
